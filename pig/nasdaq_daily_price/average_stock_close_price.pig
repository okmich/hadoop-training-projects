...
agg_data = FOREACH grp_data GENERATE group AS symbol, AVG(proj_data.stock_price_close) AS avg_closing_price;

sorted_data = ORDER agg_data BY symbol ASC;

-- store records in the ouput location
STORE sorted_data INTO '/user/cloudera/output/handson_train/nasdaq_daily_prices/avg_closing_price' USING PigStorage('|');
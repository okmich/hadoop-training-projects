...
select stock_symbol, sum(stock_volume) total_vol from tbl_nasdaq_daily_prices group by stock_symbol order by stock_symbol;

-- Find out total volume sale for each stock symbol which has closing price more than $5.
select stock_symbol, sum(stock_volume) total_volume from tbl_nasdaq_daily_prices where stock_price_close > 5.0 group by stock_symbol
...
(t.stock_price_high - t.stock_price_low) as volatility from tbl_nasdaq_daily_prices t 
join (select stock_symbol, max(stock_price_high - stock_price_low) as max_vol from tbl_nasdaq_daily_prices group by stock_symbol) v 
...
select stock_symbol, max(stock_price_close) record_closing_price from tbl_nasdaq_daily_prices group by stock_symbol
...
(select stock_symbol, max(stock_price_close) record_closing_price from tbl_nasdaq_daily_prices group by stock_symbol) prices 
join
...
select count(1) from tbl_nasdaq_daily_prices n join 
	(select t.stock_symbol, avg(t.stock_price_high - t.stock_price_low) avg_vol from 
		tbl_nasdaq_daily_prices t group by t.stock_symbol) i on i.stock_symbol = n.stock_symbol

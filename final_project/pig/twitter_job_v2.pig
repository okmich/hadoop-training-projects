...
featured_tweets = FOREACH raw_tweets GENERATE (long)json#'id' AS id, (long)json#'timestamp_ms' AS ts, (chararray)json#'lang' AS twtlang, (chararray)json#'created_at' AS created_at, encodeUTF8((chararray)json#'text') AS tweet_text, encodeUTF8(extractHref((chararray)json#'text')) AS url, encodeUTF8(extractHref((chararray)json#'source')) AS source, encodeUTF8(getHashTagText(json#'entities'#'hashtags')) AS hashtags, encodeUTF8((chararray)json#'user'#'location') AS agent_location, (chararray)json#'user'#'description' AS agent_desc, encodeUTF8((chararray)json#'user'#'name') AS agent_name, (chararray)json#'user'#'profile_image_url' AS agent_image_url, encodeUTF8((chararray)json#'user'#'screen_name') AS screen_name, (int)json#'user'#'followers_count' AS follower_count;
...
dsnt_tweets = DISTINCT featured_tweets; -- unnecessary
